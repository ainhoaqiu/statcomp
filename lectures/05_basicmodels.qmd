...
**Result.** Let $Z \sim N(\mu, V)$ be a $p$-dimensional Gaussian vector, and
$C$ a $q \times p$ matrix with full rank $q \leq p$. Then
$$
W= C Z \sim N(C \mu, C V C^T)
$$
follows a $q$-dimensional Gaussian
...
`gtsummary` has function `tbl_regression` to nicely format regression output

```{r}
tbl_regression(lmfit2) |>
  add_global_p()   # add overall P-value for categorical covariates
```
...
ggplot(diamonds2, aes(x=pred2, y=res2)) + 
  geom_boxplot(mapping = aes(group = cut_interval(pred2, n=20))) +
  labs(x='Model prediction', y='Residuals')
```
...
source("../code/routines.R")
hc= transmute(hate_crimes, state=state_abbrev, hatecrimes_fbi=avg_hatecrimes_per_100k_fbi, 
              votes_trump=share_vote_trump, gini=gini_index, hs=share_pop_hs) |>
  filter(!is.na(hatecrimes_fbi)) #outcome must be non-missing
```
...
More precisely, the model is $\mbox{lprice}_i=$
$$
\beta_0 + \beta_1 \mbox{carat}_i + \beta_2 \mbox{good}_i + \beta_3 \mbox{very good}_i + \beta_4 \mbox{premium}_i + \beta_5 \mbox{ideal}_i + \epsilon_i 
$$
...
```{r}
exp2 = function(x, ...) {  #custom function to exponentiate coef and CIs
      broom::tidy(x, ...) |>
      dplyr::mutate(estimate  = 2^estimate, conf.low  = 2^conf.low, conf.high = 2^conf.high)
}

tbl_regression(lmfit2, tidy_fun = exp2) |>
  modify_fmt_fun(estimate  ~ function(x) sprintf("%.2f", x), #format to 2 decimal places
...
- Table with regression output: `coefSummary` (at `routines.R`), `tbl_summary` (package `gtsummary`)

- Plot coefficients: `coefplot`, `multiplot` (package `coefplot`)

- Plot predictions: `data_grid`, `add_predictions` (package `modelr`)
