...
hc = select(hate_crimes, state_abbrev, avg_hatecrimes_per_100k_fbi, share_vote_trump, gini_index,share_pop_hs)
...
```{r}
states= transform(states, population_log10= log10(Population))
fit1= lm(Murder~ population_log10 + Illiteracy+Income+Frost, data=states)
coefSummary(fit1)
```
...
```{r}
fit2= lmp(Murder ~ population_log10 + Illiteracy + Income + Frost, data=states)
summary(fit2)
```
...
```{r}
y= states$Murder; x1= states$population_log10; x2= states[,c('Illiteracy','Income','Frost')]
bperm= lmResPerm.onevar(y, x1, x2, B=5000)
```

```{r}
bobs= coef(fit1)['population_log10']
...
```{r}
x= cbind(population_log10=x1, x2)
