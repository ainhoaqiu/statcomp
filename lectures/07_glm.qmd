...
```{r}
#| code-fold: true
email = mutate(email, re_subj_txt= ifelse(re_subj == 1, "Re:", "No Re:"))
ggplot(email) + 
  geom_bar(aes(x=spam, y=after_stat(prop), group=1)) + 
  facet_wrap(~ re_subj_txt) +
  labs(y= "Proportion of emails that are spam", x="Is the email spam?", title="Re: in email's subject")
```
...
```{r}
#| code-fold: true
spam_proportion <- transform(email, num_char_disc= cut_number(num_char, 5)) |>
  group_by(num_char_disc, re_subj) |>
  summarize(spam= mean(spam == '1'))

ggplot(spam_proportion, aes(x=num_char_disc, y=spam, color=re_subj)) + 
  geom_point() +
  labs(x="Number of characters", color="Re: in subject", y="Proportion of spam emails") +
...
```{r}
ggplot(fitpred, aes(num_char, pred)) + geom_line() + labs(x="Number of characters", y="Estimated spam probability")
```
...
fitpred= data_grid(emailnum, num_char, re_subj1=m, .model=fitnum) |>
  add_predictions(fitnum, type="response")
fitpred
```
...
```{r}
ggplot(fitpred, aes(num_char, pred)) + 
  geom_line() + 
...
What's the estimated odds ratio for +10 characters within:

- The group where `re_subj` is 0?

- The group where `re_subj` is 1?
