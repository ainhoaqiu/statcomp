...
Source code: `01_Basics.qmd` (quarto markdown) at <https://github.com/davidrusi/statcomp>

I recommend [GitHub Desktop](https://desktop.github.com) to pull the latest changes. See the [README](https://github.com/davidrusi/statcomp) on how to sync your local changes with the repository
...
-   Copilot access. Get a free [GitHub Student developer pack](https://education.github.com/pack)

Activate copilot in RStudio

1.  Go to Tools -\> Global Options...

2.  In the left menu, click `Copilot`. Then `Enable GitHub Copilot`

3.  Click the `Sign In` button. Follow the steps to authenticate with GitHub. Once done, click OK to close the Global Options dialog

Alternatively, use your favorite AI (e.g. ChatGPT, Gemini, etc) on the browser
...
```{r}
# Create a scatter plot of mpg vs hp using the mtcars dataset with ggplot2 in R.
```

Place the cursor at the end of the line and wait a few seconds. Copilot should suggest the code below. Press Tab to accept it.

::: panel-tabset
### Code

```{r, eval=FALSE} #eval: mostrar el código pero no ejecutarlo}
...
```{r, echo=FALSE} #ejecutar código pero no mostrarlo}
# Load necessary library
library(ggplot2)
# Load the mtcars dataset
data(mtcars)
...
[Visual Studio Code](https://code.visualstudio.com) is a free, open-source, multi-language code editor

-   Great integration with AI tools, including GitHub copilot

-   Great for advanced debugging, version control, R packages using C++

Setup instructions for [R in Visual Studio Code](https://code.visualstudio.com/docs/languages/r)
...
```{r, warning=FALSE, message=FALSE} #ocultar warnings y mensajes}
starwars
```
...
```{r}
class(z)
is.double(z)  #try also is.integer(z)
as.integer(z)
as.character(z)
```

```{r}
sel= (z > 5) #create a logical vector if numbers > 5 
sel
which(sel) #return indices of TRUE values
```
...
```{r}
any(z > 5) #true si al menos uno cumple
all(z > 5) #true si todos cumplen
```
...
Factors are designed for categorical variables. They're treated differently by many R functions, e.g. regression models
...
In the `starwars` data we saw an outlier in mass. Select the character (row) such that mass is not missing and mass \>500 kg
